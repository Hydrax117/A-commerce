...
const FarmerViewProductScreen = ({ navigation, route }) => {
  ...
  const handleDelete = (id) => {
    setIsloading(true);
    console.log(`http://192.168.43.109:3000/delete-product?id=${id}`);
    fetch(`http://192.168.43.109:3000/delete-product?id=${id}`, requestOptions)
  ...
  const fetchProduct = () => {
    setIsloading(true);
    fetch(`https://hydrax.onrender.com/all-products`, ProductListRequestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result.success) {
...
export default FarmerViewProductScreen;
