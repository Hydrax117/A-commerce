    ...
    fetch(network.serverip + "/login", requestOptions) // API call
      ...
      .then((result) => {
        if (
          ...
          } else if (result?.data?.userType == "FARMER") {
            //check the user type if the type is FARMER then navigate to Famer Dashboard else navigate to User Home
            _storeData(result.data);
            setIsloading(false);
            navigation.replace("farmerdashboard", { authUser: result.data }); // naviagte to Admin Dashboard
          } else {
            _storeData(result.data);
            setIsloading(false);
      ...
      .catch((error) => {
        setIsloading(false);
        console.log(" network error", setError(error.message));
      });
  };
      ...
      >
        <ScrollView style={{ flex: 1, width: "100%" }}>
          ...
          <View style={styles.welconeContainer}>
            <View>
              <Text style={styles.welcomeText}>Welcome to Agro Shop</Text>
              <Text style={styles.welcomeParagraph}>agriculture made easy</Text>
            </View>
            <View>
